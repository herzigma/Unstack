...
import { NormalizedPostSummary, Platform } from '../types';
...
export function Feed({ domain, onNavigate, onPostClick }: FeedProps) {
  const [posts, setPosts] = useState<NormalizedPostSummary[]>([]);
      ...
      <main className="max-w-3xl mx-auto px-6 mt-12">
        ...
        {loading ? (
          ...
          <div className="py-20 text-center text-ink-light">
            {platform === null ? (
              <>
                <p className="font-medium text-ink mb-2">This site doesn't support feed browsing.</p>
                <p>Try pasting a direct article link instead.</p>
              </>
            ) : (
          ...
          <div className="space-y-12">
            {posts.map((post, i) => (
              <motion.article
                ...
                onClick={() => onPostClick(post.canonicalUrl)}
              >
